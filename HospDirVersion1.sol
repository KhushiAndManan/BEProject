...
contract hospDir{
    ...
    function addDoc(/*uint id,*/ string memory _firstName, string memory _middleName, string memory _lastName) public{
        ...
        docArray.push(docDetails(_firstName, _middleName, _lastName));
    ...
    function getDetails(uint no) public view returns (string memory, string memory , string memory , string memory , string memory , string memory, uint ) {
        return (hospDeets[no].hospName, hospDeets[no].firstLine,
